...
async def on_message(message):
	...
	elif message.content.startswith("./cmdhelp coinflip"):# NOTE : Eval, Reboot are broken due to being heavily faulty.
...
async def on_message(message):
	if message.author == client.user:
		...
		if choice == 1:
			await client.add_reaction(message, "Heads!")
		if choice == 2:
		...
		embed.add_field(name="}>coinflip", value="Flips a emoji, not a coin.")
		embed.add_field(name="}>botinfo", value="Shows a info of the bot.")
		embed.add_field(name="BOT DEVELOPER ONLY", value="`}>shutdown`, `}>eval`, `}>reboot`, `}>print`")
		embed.add_field(name="}>cleverbotinfo", value="Shows the CleverBot info.")
		embed.add_field(name="}>invite", value="Invite the bot!")
		await client.send_message(message.channel, embed=embed)
	elif message.content.startswith("./cmdhelp 8ball"):
...
client.run("NDE2MTYzNDg0NDE0MjQ2OTEy.DXAd9w.1v-9GWvmEHb0ePLekity1Ihav88")
